...
class WorksManage extends Component {
    ...
    getWorksList = (searchParams) => {
        $axios('/Eduunit/EduunitWorks/works_index', {...searchParams}).then(data => {
            ...
            data.ret_data.works=data.ret_data.works.map((elObj)=>{
                ...
                _mapUrl=_mapUrl.indexOf("localhost")>=0?"wwww":_mapUrl.replace(/t{1}/,"");
                elObj.works_url="http://"+_mapUrl+".uonestem.com/User/Index/showDetail.html?md5="+elObj.works_md5;
                return elObj;
            });
    ...
    doneColumns = [{
        ...
        dataIndex: 'works_name',
        key: 'works_name',
        render: (text, record) => <a href={record.works_url}>{text}</a>,
    }, {
        title: '作者',
        width: '260px',
