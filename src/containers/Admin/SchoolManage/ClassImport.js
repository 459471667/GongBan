...
export default class ClassImport extends React.Component {
    ...
    render() {
        ...
        const styleMap={
            student_import:"580px",
            teacher_import:"440px"
        };
                            ...
                            <tbody>
                                ...
                                {studentOrTeacherDataList.map((dataEl,index)=>{
                                    ...
                                    return (<tr key={dataEl["_key"]}>
                                                {dataObjKeys.map((_key,index)=>{


                                                    if(_key==="_key")return null;
                                                    if(_key==="data_msg")return (<td style={colorRed} key={_key}>{dataEl["data_msg"]}</td>)
                                                    //if(typeof dataEl[_key] === "object" ) return (<td style={colorRed} key={_key}>数据错误</td>)
        ...
        return (<Layout className="class-import" style={{overflowY:"hidden",  paddingLeft: "20px"}}>
                <h3>导入{titleText}</h3>
